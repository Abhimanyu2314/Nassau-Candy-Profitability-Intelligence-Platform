...
[data-testid="stSidebar"]::before { display:none !important; content:none !important; }
...
/* PHASE 54: compact sidebar status panel and no empty sidebar placeholder */
.sidebar-status-card{
    margin:14px 0 8px 0;
    padding:14px 13px;
    border-radius:18px;
    background:linear-gradient(145deg, rgba(255,255,255,.055), rgba(236,72,153,.06));
    border:1px solid rgba(236,72,153,.28);
    box-shadow:0 12px 30px rgba(0,0,0,.22), inset 0 1px 0 rgba(255,255,255,.06);
}
.sidebar-status-card .status-title{
    color:#fff;font-weight:850;font-size:.92rem;margin-bottom:10px;letter-spacing:.15px;
}
.sidebar-status-card .status-row{
    display:flex;justify-content:space-between;gap:10px;padding:6px 0;
    border-bottom:1px solid rgba(255,255,255,.07);font-size:.78rem;
}
.sidebar-status-card .status-row:last-child{border-bottom:none;}
.sidebar-status-card .status-label{color:#bdaed1!important;}
.sidebar-status-card .status-value{color:#fff!important;font-weight:800;text-align:right;}
...
with st.sidebar:
    ...
    try:
        _top_division = str(raw.groupby("Division")["Gross Profit"].sum().idxmax()) if "Division" in raw.columns and "Gross Profit" in raw.columns and not raw.empty else "N/A"
        _top_factory = str(raw.groupby("Factory")["Gross Profit"].sum().idxmax()) if "Factory" in raw.columns and "Gross Profit" in raw.columns and not raw.empty else "N/A"
        _status_html = f"""
        <div class='sidebar-status-card'>
            <div class='status-title'>🟢 System Status</div>
            <div class='status-row'><span class='status-label'>Records</span><span class='status-value'>{len(raw):,}</span></div>
            <div class='status-row'><span class='status-label'>Top Division</span><span class='status-value'>{_top_division}</span></div>
            <div class='status-row'><span class='status-label'>Top Factory</span><span class='status-value'>{_top_factory}</span></div>
            <div class='status-row'><span class='status-label'>Version</span><span class='status-value'>v54.0</span></div>
        </div>
        """
        st.markdown(_status_html, unsafe_allow_html=True)
    except Exception:
        st.markdown("<div class='sidebar-status-card'><div class='status-title'>🟢 System Status</div><div class='status-row'><span class='status-label'>Version</span><span class='status-value'>v54.0</span></div></div>", unsafe_allow_html=True)
...
rail_html += "<div class='rail-spacer'></div><div class='rail-version'>v54.0</div></div>"
...
st.success("Phase 54 active: faster loading, smoother export center, lazy report generation, and optimized transition performance.")
...
<div class='neon-footer'>
  🍬 Nassau Candy Profitability Intelligence Platform • Enterprise Analytics Suite v54.0 • Powered by Streamlit • Last Refresh: 07 Jun 2026
</div>
""",
    unsafe_allow_html=True,
)
...
st.markdown("""
<style>
/* PHASE 54 FINAL SIDEBAR CLEANUP */
[data-testid="stSidebar"]::before{display:none!important;content:none!important;height:0!important;margin:0!important;padding:0!important;}
